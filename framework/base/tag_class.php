		...
		if($matches[1]!=='/')
		{
			switch($matches[2].$matches[3])
			{
                ...
                case '#':{
                    $str = trim($matches[4]);
                    if ($str == '_theme'){
                        return '<?php $_themeName =  Core::app()->getTheme()->getName();$model= new Model("theme_config"); $_themeObj=$model->where("name='."'".'$_themeName'."'".'")->find();if(isset($_themeObj["config"])){$_themeConfig = $_themeObj["config"]; $_themeConfig = unserialize($_themeConfig);extract($_themeConfig,EXTR_SKIP);}?>';
                    }else{
				...
				case 'dump:':return '<pre><?php Core::dump('.$matches[4].'); ?></pre>';
				case 'list:':
				case 'foreach:':
				{
                ...
                case 'widget:':
                {
					...
					$new_char=array(' != ',' = ',' < ',' > ',' <= ',' >= ');
					$tem = "<?php \$widget = Widget::createWidget('$className');";
					foreach($attr as $k => $v)
					{
						if($k != 'name'){
				...
				case 'token:':
				{
					...
					return "<input type='hidden' name='labor_token_".$key."' value='<?php echo Core::app()->getToken(\"".$key."\");?>'/>";
				}
				case 'debug:':
				{
				...
				case 'include:':
				{
					...
					$viewfile = Core::app()->getViewPath().DIRECTORY_SEPARATOR.$this->viewPath.DIRECTORY_SEPARATOR.$fileName;
					$runfile= Core::app()->getRuntimePath().DIRECTORY_SEPARATOR.$this->viewPath.DIRECTORY_SEPARATOR.$fileName;
