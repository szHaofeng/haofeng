...
class HTMLPurifier_HTMLDefinition extends HTMLPurifier_Definition
{
    ...
    protected function setupConfigStuff($config)
    {
        ...
        if (!is_array($allowed_elements) && !is_array($allowed_attributes)) {
            ...
            if (is_string($allowed)) {
                list($allowed_elements, $allowed_attributes) = $this->parseLaborMCEAllowedList($allowed);
            }
        }
    ...
    /**
     * Parses a LaborMCE-flavored Allowed Elements and Attributes list into
     * separate lists for processing. Format is element[attr1|attr2],element2...
     * @warning Although it's largely drawn from LaborMCE's implementation,
     *      it is different, and you'll probably have to modify your lists
     * @param array $list String list to parse
     * @return array
     * @todo Give this its own class, probably static interface
     */
    public function parseLaborMCEAllowedList($list)
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            // remove LaborMCE element control characters
            if (!strpos($chunk, '[')) {
                $element = $chunk;
                $attr = false;
            } else {
