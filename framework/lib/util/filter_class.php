...
final class Filter
{
	...
	*/
	...
	{
	    $cache_dir=Core::getPath('cache')."/htmlpurifier/";
	...
	public static function sql($str)
	{
		...
		$str = preg_replace('/([^a-z]+)(select|insert|update|delete|union|into|load_file|outfile|and|or|sleep|Core_)/i', '&#160;$2', $str);
	...
	public static function inputsform($inputs) {
		$rule=array();
		foreach ($inputs as $key => $input) {
			$rule=array_merge($rule,array($input["type"]=>$input["field"]));
		}
		self::form($rule);
	}
}
