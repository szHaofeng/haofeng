...
class Http
{
    ...
    public static function doGet($url,$timeout=5,$header="") 
    {
        ...
        switch($code)
        {
            ...
            case 2:
                ...
                break;
            case 3:return 
                self::phpGet($url,$timeout,$header);
                break;
    ...
    public static function zip($files,$zipfile)
    {
    ...
    //下载导出CSV文件
    public static function exportCSV($headings=false, $rows=false, $filename=false)
    {
        if (!empty($rows)){
    ...
    private static function getSupport()
    {
        //如果指定访问方式，则按指定的方式去访问
    ...
    //通过curl get数据
    private static function curlGet($url,$timeout=5,$header="")
    {
    ...
    private static  function addReferfer(&$header,$url){
        $referer = preg_replace('/((?<!\/)\/(?!\/).*)$/i','',$url);
        $header .= "Referer:$referer\r\n";
    }
    //通过curl post数据
    private static  function curlPost($url, $post_data=array(), $timeout=5,$header="")
    {
    ...
    //通过socket get数据
    private static  function socketGet($url,$timeout=5,$header="")
    {
        $header=empty($header)?self::defaultHeader():$header;
        self::addReferfer($header,$url);
        $url2 = parse_url($url);
    ...
    //通过socket post数据
    private static  function socketPost($url, $post_data=array(), $timeout=5,$header="")
    {
        $header=empty($header)?self::defaultHeader():$header;
        self::addReferfer($header,$url);
        $post_string = http_build_query($post_data);
    ...
    private static  function phpGet($url,$timeout=5,$header="")
    {
        $header=empty($header)?self::defaultHeader():$header;
        self::addReferfer($header,$url);
        $opts = array(
    ...
    private static  function phpPost($url, $post_data=array(), $timeout=5,$header="")
    {
        $header=empty($header)?self::defaultHeader():$header;
        self::addReferfer($header,$url);
        $post_string = http_build_query($post_data);
