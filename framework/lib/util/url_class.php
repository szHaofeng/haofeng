...
class Url
{
    ...
    public static function getRefRoute()
    {
        ...
        if(isset($_SERVER['HTTP_REFERER'])){
            return substr($_SERVER['HTTP_REFERER'],strlen(self::getHost().self::baseUri()));
    ...
    static function urlFormat($path)
    {
        ...
        switch(substr($path,0,1))
        {
            ...
            case '#': //解析成主题的绝对地址
            {
                if(Core::app()->getTheme()!==null)
                {
                    return Core::app()->getTheme()->getBaseUrl().'/'.substr($path,1);
                }
                else return self::baseDir().substr($path,1);
            }
            case '*'://解析成皮肤的路径
            {
                if(Core::app()->getTheme()!==null && Core::app()->getSkin()!==null)
                {
                    $theme = Core::app()->getTheme();
                    return  $theme->getBaseUrl().'/skins/'.Core::app()->getSkin().'/'.substr($path,1);
                }
                else if( Core::app()->getSkin()!==null){
                    return self::baseDir().'skins/'.Core::app()->getSkin().'/'.substr($path,1);
                }
                else return self::urlFormat('#'.substr($path,1));
            }
            case '!'://运行目录路径
            {
                return Core::app()->getRuntimeUrl().'/'.substr($path,1);
            }
            default:
            {
                $q = Req::get();
