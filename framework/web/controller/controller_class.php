...
class Controller extends Bobject
{
    ...
    {
        if(($theme = Core::app()->getTheme())!==null && ($viewFile=$theme->getViewFile($this,$viewName))!==null)
        {
    ...
    public function getLayoutFile()
    {
        ...
        if(($theme = Core::app()->getTheme())!==null && ($layoutFile = $theme->getLayoutFile($this,$layoutName))!==null)
        {
    ...
    public function captcha()
    {
        ob_start();
        $this->layout = null;
        $w=Req::args('w')===null?100:intval(Req::args('w'));
        $h=Req::args('h')===null?30:intval(Req::args('h'));
        $l=Req::args('l')===null?4:intval(Req::args('l'));
        $bc=Req::args('bc')===null?'#1a78c2':'#'.Req::args('bc');
        $c=Req::args('c')===null?'#ffffff':'#'.Req::args('c');
    ...
    public function getRunFile($view){
        //return Core::app()->getRuntimePath().DIRECTORY_SEPARATOR.$this->getId().DIRECTORY_SEPARATOR.$view.$this->scriptExt;
        return Core::app()->getRuntimePath().DIRECTORY_SEPARATOR.$this->getId().DIRECTORY_SEPARATOR.$view.$this->scriptExt;
    ...
    public function renderExecute($__runfile0123456789,$__data0123456789)
    {
            ...
            $__datas0123456789 += Core::app()->getDatas()!==null?Core::app()->getDatas():array();
    ...
    {
        $extension='.html';
        return Core::app()->getViewPath().DIRECTORY_SEPARATOR.$viewName.$extension;
    }
    ...
    public function redirect($operator ='',$jump=true,$args=array())
    {
        ...
        else//真跳转处理,包括参数传递
        {
            ...
            if($old_args_num!=0 && is_array($args) && !empty($args)){
                $args['labor_token_redirect'] = Core::app()->getToken('redirect');
                //var_dump($args);exit();
                header("Content-type: text/html; charset=".$this->encoding);
