...
class Model
{
    ...
    public function join($str)
    {
        ...
        $str = strtolower($str);
        if(strpos($str,'join')===false) $this->sql['join'] = 'left join '.preg_replace('/(\w+)(?=\s+as\s+\w+(,|\)|\s))/i',"$1 ",$str);
        else $this->sql['join'] = preg_replace('/(\w+)(?=\s+as\s+\w+(,|\)|\s))/i',"$1 ",$str);
        return $this;
    }
    /**
     * 条件信息
     ...
     * @return source
     */
    public function findPage($page, $list_num=30,$type=1,$ajax=false,$ajaxFunction='')
    {

        $list_num = $list_num>0?(int)$list_num:30;
        $sql = $this->sql;
     ...
     * @return void
     */
    public function getPage($page, $list_num=30)
    {
        //参数分析
        $page		= (int)$page;
    ...
    {
        $sql = $this->sql;
        $sqlStr = "truncate table {$sql['table']}";
        return $this->query($sqlStr);
    }
    /**
     * 保存数据，自动识别更新还是添加
    ...
    private function changeWhere()
    {
        ...
        if(is_array($sql['data']))
        {
            ...
            if (in_array($this->primary_key, $key_array) && isset($sql['data'][$this->primary_key]))
            {
