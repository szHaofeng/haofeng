...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-indextop="{{ '+ (options.indextop||'i') +' }}" data-index="{{ '+ (options.index||'i') +' }}" id="layim-'+ options.type +'{{ data.id }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'</div>'
    ,'<ul class="layui-layim-tab{{# if((!d.base.isfriend || !d.base.isgroup) && !d.base.isgovlist){ }}'
      ,' layim-tab-two'
    ,'{{# } }}">'
      ,'<li class="layui-icon'
      ,'{{# if(!d.base.isfriend){ }}'
      ...
      ,'{{# if(!d.base.isgovlist){ }}'
      ,' layim-hide'
      ,'{{# } else if(!d.base.isfriend && !d.base.isgroup) { }}'
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="政务" layim-event="tab" lay-type="govlist">&#xe613;</li>'
    ...
    ,'</ul>'
     //拓展------------------------------------------------------------------------------------------
     ,'<ul class="layim-tab-content {{# if(!d.base.isfriend && !d.base.isgroup && d.base.isgovlist){ }}layim-show{{# } }} layim-list-govlist">'
     ,'{{# layui.each(d.govlist, function(index, item){ var spread = d.local["spread"+index]; }}'
     ,'{{# if(item.list.length > 0){ }}'
        ,'<li>'
          ...
          ,'<ul data-index="{{ index }}" class=" {{# if(spread === "true"){ }}'
          ,' layim-show'
          ,'{{# } }}" style="margin-left: 10px;height:auto;">'
          ,'{{# layui.each(item.list, function(indexs, items){ var spreads = d.local["spread"+indexs]; }}'
          ,'{{# if(items.list.length > 0){ }}'
              ,'<li>'
              ,'<h5 layim-event="spread" lay-type="{{ spreads }}"><i class="layui-icon">{{# if(spreads === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ items.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ (items.list||[]).length }}</cite>)</em></h5>'
              ,'<ul data-index="{{ indexs }}" class="layui-layim-list {{# if(spreads === "true"){ }}'
              ,' layim-show'
              ,'{{# } }}">'
              ,listTpl({
                      type: "govlist"
  ...
  //转换内容
  layui.data.content = function(content,sdata){
    //支持的html标签
    ...
    .replace(/img\[([^\s]+?)\]/g, function(img){  //转义图片
      ...
      var txid = img.replace(/(^img\[)|(\]$)/g, '');

      if(sdata){
        if(sdata.send_type==1){
          return '<a class="layui-layim-file" href="javascript:;" title="文件下载" onclick=\"down('+sdata.address_id+',\'' + txid + '\')\" ><cite>'+ txid +'</cite></a>';
         }else{
           var href =sdata.send_href+txid;
          return '<a class="layui-layim-file" href="'+href+'" target="_blank" title="文件下载"><cite>'+ txid +'</cite></a>';
         }
      }else{
        if(cache.base.send_type==1){
          return '<a class="layui-layim-file" href="javascript:;" title="文件下载" onclick=\"down('+cache.base.address_id+',\'' + txid + '\')\" ><cite>'+ txid +'</cite></a>';
         }else{
           var href =cache.base.send_href+txid;
          return '<a class="layui-layim-file" href="'+href+'" target="_blank" title="文件下载"><cite>'+ txid +'</cite></a>';
         }
      }
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      //,content: '<img src="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title||'') +'11</span>'
      ,content: '<div class="fa fa-comments" style="font-size: 26px; display: block;line-height: 30px;text-align: center;"></div><span style="margin: 0px;padding: 0px;line-height: 16px;font-size: 12px;">'+ (content||cache.base.title||'聊天') +'</span>'
  ...
  var getMessage = function(data){
    ...
    if((!layimChat && data.content) || index === -1){
      if(cache.message[data.type + data.id]){
        ...
        }else if(data.type === 'govlist'){
          var govlist;
          layui.each(cache.govlist, function(index1, item1){
            layui.each(item1.list, function(index, item){
              if(item.id == data.id){
                item.type = 'govlist';
                item.name = item.username;
  ...
  var addList = function(data){
    ...
    var list = laytpl(listTpl({
      type: data.type
      ,item: 'd.data'
      ,index: data.type === 'friend' ? 'data.groupIndex' : 'govlist'
    })).render({data: obj});
    ...
    if(data.type === 'friend'){
      ...
      var li = listElem.find('>li').eq(data.groupIndex);
      li.find('.layui-layim-list').append(list);
      li.find('.layim-count').html(cache.govlist[data.groupIndex].list.length); //刷新好友数量
      //如果初始没有好友
      if(li.find('.layim-null')[0]){
        li.find('.layim-null').remove();
      }
  ...
  var anim = 'layui-anim-up', events = {
    ...
    ,search: function(othis){
      ...
      var input = search.find('input'), find = function(e){
        ...
        if(val === ''){
          ...
          var group = cache.group || [], html = '',govlist = cache.govlist || [];
          ...
          for(var ii = 0; ii < govlist.length; ii++){
            for(var kk = 0; kk < (govlist[ii].list||[]).length; kk++){
              if(govlist[ii].list[kk].groupname.indexOf(val) !== -1){
                for(var jj = 0; jj < (govlist[ii].list[kk].list||[]).length; jj++){
                  if(govlist[ii].list[kk].list[jj]){
                    govlist[ii].list[kk].list[jj].type="govlist";
                    govlist[ii].list[kk].list[jj].indextop=ii;
                    govlist[ii].list[kk].list[jj].index=kk;
                    govlist[ii].list[kk].list[jj].list=jj;
                    data.push(govlist[ii].list[kk].list[jj]);
                  }
                }
              }
            
              for(var jj = 0; jj < (govlist[ii].list[kk].list||[]).length; jj++){
                if(govlist[ii].list[kk].list[jj].username.indexOf(val) !== -1){
                  govlist[ii].list[kk].list[jj].type="govlist";
                  govlist[ii].list[kk].list[jj].indextop=ii;
                  govlist[ii].list[kk].list[jj].index=kk;
                  govlist[ii].list[kk].list[jj].list=jj;
          ...
          if(cache.base.send_type==1){
            if(data.length > 0){
              for(var l = 0; l < data.length; l++){
                html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-indextop="'+ data[l].indextop +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
              }
            } else {
           ...
           //cache = $.extend(cache, obj);
            var searchurl =  cache.base.searchurl;
                $.ajax({
                  type: 'POST',
                  url: searchurl,
                  data: {
                  ...
                  success:function(data){
                    if(data.length > 0){
                      cache = $.extend(cache,{friendata:data});
                      for(var l = 0; l < data.length; l++){
                        html += '<li layim-event="chat" data-type="friend" data-index="0" data-list="'+ l +'" id="friend'+data[l].id+'"><img src="'+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
                      }
                    } else {
                      html = '<li class="layim-null">无搜索结果</li>';
                    }
      ...
      if(!cache.base.isfriend && cache.base.isgroup && !cache.base.isgovlist){
        events.tab.index = 1;
      } else if(!cache.base.isfriend && !cache.base.isgroup && !cache.base.isgroup){
        events.tab.index = 2;
      }
    ...
    ,closeSearch: function(othis){
      cache = $.extend(cache,{friendata:0});
      othis.parent().hide();
      events.tab(events.tab.index|0);
    }
    ...
    ,chat: function(othis){
      var local = layui.data('layim')[cache.mine.id] || {};
      var type = othis.data('type'), index = othis.data('index'); indextop = othis.data('indextop');
      var list = othis.attr('data-list') || othis.index(), data = {};
     
      if(cache['friendata']==0 || !cache['friendata']){
        if(type === 'friend'){
        ...
        } else if(type === 'govlist'){
          data = cache[type][indextop].list[index].list[list];
        }
        
      }else{
         //自定义查询
          data =cache['friendata'][list];
      }
      data.name = data.name || data.username || data.groupname;
      if(type !== 'history'){
        data.type = type;
    ...
    ,chatLog: function(othis){
      ...
      if(cache.base.urlFormat=='get'){
        logurl= cache.base.chatLog + '&id=' + thatChat.data.id + '&type=' + thatChat.data.type+ '&address_id=' + cache.base.address_id;
      }else{
        logurl= cache.base.chatLog + '/id/' + thatChat.data.id + '/type/' + thatChat.data.type+ '/address_id/' + cache.base.address_id;
      }
